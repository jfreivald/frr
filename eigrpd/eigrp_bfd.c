...
{
	uint32_t t = 0;
	if ((s)->SessionState == EIGRP_BFD_STATUS_UP) {
		if ((s)->bfd_params->DesiredMinTxInterval > (s)->bfd_params->RemoteMinRxInterval) {
			t = (s)->bfd_params->DesiredMinTxInterval;
		} else {
			t = (s)->bfd_params->RemoteMinRxInterval;
		}
	} else {
...
static int eigrp_bfd_process_ctl_msg(struct stream *s, struct eigrp_interface *ei, struct in_addr *client_address) {
    ...
    if (session->SessionState == EIGRP_BFD_STATUS_DOWN) {
        ...
        if (bfd_msg->flags.sta == EIGRP_BFD_STATUS_DOWN) {
            L(zlog_debug, LOGGER_EIGRP, LOGGER_EIGRP_PACKET, "BFD: Session transitioned DOWN. Tear down neighbor.");
            session->header.diag = EIGRP_BFD_DIAG_NBR_SESSION_DWN;
            session->SessionState = EIGRP_BFD_STATUS_DOWN;
            eigrp_nbr_down(nbr);
        }
	...
	struct timeval tv;
    	...
    	tv.tv_usec = detection_timer % 1000000;

    	thread_add_timer_tv(master, eigrp_bfd_session_timer_expired, session, &tv,
